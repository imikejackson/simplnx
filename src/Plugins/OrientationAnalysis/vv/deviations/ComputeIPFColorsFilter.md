...
**Release-matrix evidence (2026-09-17):** NX 7.4.1 with EbsdLib 2.2.0 and NX 7.4.2 with EbsdLib 3.1.2 are byte-identical across all 343,963 RGB tuples and both reproduce the embedded legacy reference exactly. Each differs from fresh official DREAM3D 6.5.171 in the same 14 tuples, one component per tuple, with maximum channel delta 1. The result is stable across both shipped NX/library combinations and requires no patch experiment because neither quantization direction is incorrect.
