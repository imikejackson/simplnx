...
The vMF/Watson EM math is owned and tested by EbsdLib (`EbsdLib/Source/Test/DirectionalStatsTest.cpp`). EbsdLib's `ebsdlib::DirectionalStats` is itself a C++ port of the EMsoft `dictmod.f90` directional-statistics module (Marc De Graef Research Group / Carnegie Mellon University, BSD 3-Clause; original EM implementation by Yu-Hui Chen), introduced by EbsdLib PR #41 / commit `006a023` (2026-04-03). `DirectionalStats::EMforDS` corresponds to EMsoft's `DI_EMforDD`, and the estimator is the modified (symmetry-group-invariant) vMF / axial Watson form published in Chen *et al.*, *Microsc. Microanal.* **21**(3), 739-752 (2015) and *IEEE Signal Process. Lett.* **22**(8), 1152-1155 (2015). The reference is therefore a published, third-party algorithm with an independently maintained implementation — not self-generated output — which is what makes it admissible as a Class 2 oracle. Its `DirectionalStatsTest:VMF` / `:Watson` cases run **at this filter's exact configuration** (`numEM=5, numIter=10, seed=43514`, Cubic_High ops) over a fixed 22-quaternion set (`detail::k_TestQuats`, each FZ-reduced via `cubicOps->getFZQuat`), and hard-assert the EM result. The filter performs the identical pipeline (`getFZQuat` → `EMforDS(43514,…)` → `positiveOrientation`).
