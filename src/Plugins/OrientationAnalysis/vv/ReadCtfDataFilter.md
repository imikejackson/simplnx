...
| Status | COMPLETE |
| Sign-off | Michael A. Jackson <mike.jackson@bluequartz.net> — 2026-07-24 |
| Second-engineer sign-off | Jared Duffey — 2026-07-28 (approving reviewer, PR #1692) |

## At a glance

| Aspect                 | Current state            |
|------------------------|--------------------------|
| Algorithm Relationship | **Minor changes** to legacy `ReadCtfData`: preserve phase 0, restore 3D import, add malformed-input guards, use double Euler intermediates, and remove the legacy cache. |
| Oracle (confirmed)     | **Classes 1 and 4** use inline 2D and 3D `.ctf` fixtures with hand-derived values and independently rounded angle transforms; EbsdLib parsing is the trusted Class 2 boundary. All 12 tests pass. |
| Code paths enumerated  | 19 of 22 paths exercised; unreadable-header, file-mutation, and cancel paths require injection. |
| Tests today            | 12 test cases cover analytical values, Euler options, 3D import, error guards, EbsdLib passthroughs, and SIMPL conversion. |
| Exemplar archive       | None; legacy-generated `6_6_read_ctf_data_2.tar.gz` was retired as an oracle and replaced by inline fixtures. |
| Legacy comparison      | **Run** — well-formed outputs are bit-identical except D1/D2; D3 records malformed-input failures, and D4 records NX multi-slice truncation through 7.4.1. Controlled proofs confirm the D3 guards and the D4 fix in 7.4.2. |
| Bug flags              | `ReadCtfDataFilter-D3` — unsafe malformed-input access; `ReadCtfDataFilter-D4` — DREAM3D-NX truncated multi-slice input through 7.4.1. |
| V&V phase | **COMPLETE.** |

## Summary

`ReadCtfDataFilter` ("Read Oxford Instr. EBSD Data (.ctf)") imports a single Oxford/HKL Channel 5 `.ctf` file into a new Image Geometry: it builds the geometry from the header (XCells/YCells/ZCells, XStep/YStep/ZStep, origin 0, Micrometer), creates one cell array per data column plus the condensed 3-component `EulerAngles` and verbatim `Phases` arrays (optionally applying the EDAX hexagonal +30° φ2 alignment and degrees→radians conversion), and populates the ensemble arrays (`CrystalStructures` via EbsdLib's Laue-group mapping, `MaterialName`, `LatticeConstants`) with slot 0 reserved for the "Invalid Phase". Verification is Class 1 analytical + Class 4 invariant on hand-authored inline toy `.ctf` fixtures whose expected outputs were derived independently of both codebases (EbsdLib parsing is trusted upstream and not re-tested). Headline result: SIMPLNX matches the oracle exactly; against DREAM3D 6.5.171 every numeric output is bit-identical on all four fixtures except the deliberate unindexed-point deviations (D1/D2), with one SIMPLNX functional gap (3D multi-slice import, D4) found, release-matrix proven, and resolved, and legacy's malformed-input crashes (D3) empirically demonstrated and patch-proven. All 12 unit tests pass; the legacy-oracle exemplar archive `6_6_read_ctf_data_2.tar.gz` is retired.
...
2. **3D multi-slice support restored** (D4, resolved). Legacy sizes the geometry `XCells × YCells × ZCells` with `ZStep` slice thickness; the NX port had hard-coded z=1/1.0 and silently imported only slice 0. Preflight now mirrors legacy (`ZCells` clamped to ≥1; `ZStep` of 0 → 1.0). Bit-identical to legacy on the 3D fixture.
...
7. **Legacy PIMPL file-cache dropped** (`ReadCtfDataPrivate`/`Ctf_Private_Data` — ported as dead declarations, removed). No output effect; NX re-reads the file on each execute.
...
*Applied:* Hand-authored 2D and 3D `.ctf` fixtures provide exact expected geometry and arrays. Independently calculated IEEE-754 values verify the optional Euler transforms.
...
|-----------|--------|----------------------------|---------------------------|
| `ReadCtfDataFilter-D3` | Missing arrays, empty phase lists, and out-of-range phase values can cause crashes or out-of-bounds access. | DREAM.3D 6.5.171; *Affected DREAM3D-NX versions not recorded in the deviation record* | Add explicit guards with errors `-19600` through `-19604`; well-formed output is unchanged. |
| `ReadCtfDataFilter-D4` | The SIMPLNX port ignored `ZCells` and `ZStep` and imported only slice 0. | DREAM3D-NX v7.0.0 through v7.4.1. DREAM.3D 6.5.171 was not affected. | Restore the header z extent and spacing; DREAM3D-NX 7.4.2 imports the full volume. |
...
- **Write pattern:** all destination writes are forward-sequential; the Euler loop's re-read of the just-written Phases array was replaced with the reader's in-memory buffer (bit-identical, removes a redundant read-back); `copyFromBuffer`-style conversions deferred until that API is available.

## Code path coverage

*19 of 22 enumerated paths exercised; the gaps are one passthrough needing permission manipulation (row 1), the file-changed phase-count guard (row 9b), and the cancel checks (row 21).*

Source: `src/Plugins/OrientationAnalysis/src/OrientationAnalysis/Filters/Algorithms/ReadCtfData.cpp` (284 lines) + preflight in `Filters/ReadCtfDataFilter.cpp` (268 lines).
...
| `…: 3D Multi-Slice CTF` | new-for-V&V | Class 1. DYNAMIC_SECTION (ZStep 0.75 / absent). Regression pin for D4 (3D support restored). |
...
- **Archive:** None. **`6_6_read_ctf_data_2.tar.gz` retired** (SHA512 was `f397fa3b…ad1707`); its `download_test_data()` entry is removed from `test/CMakeLists.txt`.
...
*Comparison run 2026-07-24 against the official DREAM3D 6.5.171 release on four byte-identical-input fixtures (toy ×2 conversion combos, Cugrid 550×400 production scan, 2×2×2 multi-slice toy) plus three malformed-input fixtures, followed by controlled legacy-patch and NX-release proofs on 2026-09-17. All numeric outputs bit-identical outside the entries below. Full write-ups: `vv/deviations/ReadCtfDataFilter.md`; working artifacts archived to OneDrive (comparison working folder).*
...
- `ReadCtfDataFilter-D3` — Malformed-input behavior: legacy segfaults (missing column; empty phases — both demonstrated, exit 139) or silently emits heap-dependent output (out-of-range phase value — demonstrated, exit 0); SIMPLNX rejects with −19600…−19604. Three isolated local legacy guards produce the predicted clean errors −19600/−19601/−19602 without altering any of 60 well-formed baseline datasets. Trust SIMPLNX.
- `ReadCtfDataFilter-D4` — *(retired)* Multi-slice `.ctf` is truncated to slice 0 by NX 7.4.1/EbsdLib 2.2.0; NX 7.4.2/EbsdLib 3.1.2 restores the full volume and matches legacy Euler values bit-for-bit on all eight cells. Trust SIMPLNX (current).
