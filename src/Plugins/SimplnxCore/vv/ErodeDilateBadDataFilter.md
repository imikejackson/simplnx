...
5. **Cancel check** — SIMPLNX reads `m_ShouldCancel` once per Z-slice during the read-only Feature ID scan, and once more before the first morphology pass. The passes change `FeatureIds` and the sibling cell arrays in place, so after the first write they run to completion; a cancel never leaves the input partly modified. Legacy has no cancel check at all. Additive; no output change on a run to completion.
6. **Progress reporting** — SIMPLNX announces each phase with an info message and reports per-Z-slice progress through one `ThrottledMessageHandler`; legacy used `notifyStatusMessage`. No output change.
...
| 11 | (b) Cancel | `m_ShouldCancel` read once per Z-slice in the read-only Feature ID scan, and once before the first morphology pass → early return with the input unchanged. Not read during the passes, which modify the input in place | *Not directly tested. Requires cancel-signal injection; no test sets `m_ShouldCancel` and asserts early termination.* Legacy has no cancel check at all, so SIMPLNX is ahead of legacy here — not a deviation. |
...
- **One progress throttle behind a mutex.** Every progress update goes through `sendThreadSafeProgressMessage()`, which takes `m_ProgressMessage_Mutex` before it calls the single `ThrottledMessageHandler`.
